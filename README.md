...
- 支持薄弱点复盘闭环：AI 生成复盘问题、用户作答、标记掌握和掌握积分。
- 首页展示今日薄弱点，方便用户进入系统后直接复盘。
- 支持生成学习计划，并在首页勾选任务执行状态。
- 支持任务积分机制，完成学习任务后累计个人积分并形成学习等级。
...
今日薄弱点会优先展示当天自动沉淀的薄弱点；如果当天没有记录，则展示最近需要复盘的薄弱点。

学习计划可以在首页直接勾选完成状态，也可以删除已有计划。任务完成后会根据难度获得 1 到 5 积分；积分不会因为重新生成学习计划而清空，会记录在当前用户的学习档案中。
...
- 只允许删除当前用户上传的个人资料。
- 系统内置资料不会显示删除入口，也不能通过删除接口删除。
- 删除个人资料后，后端会重建知识库索引，避免 RAG 继续命中已删除资料。
...
- 薄弱点第一次标记为“已掌握”时获得 2 积分。
- 同一条薄弱点不会重复获得复盘积分。
- 只保存复盘回答或改为“复盘中”不会加分。
- 已掌握的薄弱点不会优先出现在首页“今日薄弱点”中。
...
- 学习等级：根据累计积分展示当前等级和升级进度。
- 状态总览：累计提问、待复盘数量、进行中任务、当前方向。
...
- 每个任务根据难度分配 1 到 5 积分。
- 用户首次完成任务时获得积分。
- 已获得积分的任务再次取消/勾选不会重复加分。
- 积分记录在当前用户的学习档案中，不会随新计划刷新。

阶段总结：

- 当一个学习计划完成度达到 100% 后，首页会显示“生成总结”入口。
- 总结会整理阶段收获、关键知识点和下一步建议。

## 前端界面风格

当前前端采用简约清新的卡片风格，重点是让核心学习流程更清晰：

- 全站统一圆角、阴影、浅色渐变背景。
- 首页突出“今日薄弱点”和“我的学习计划”。
- 知识库卡片支持资料选择、个人资料标识和个人资料删除。
- 薄弱点卡片显示复盘状态，并通过弹窗完成复盘笔记和标记掌握。
- 登录注册、知识库、学习档案、薄弱点页面去除了无意义英文装饰标签。
- 空状态、成功提示、按钮反馈采用统一样式，减少原型感。
